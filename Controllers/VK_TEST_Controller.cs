...
namespace VK_TEST.Controllers
{
    ...
    public class VK_TEST_Controller : ControllerBase
    {
        ...
        private void ThrowAdmin()
        {
            ...
            if (group.Code != UserGroup.AllGroups[1].Code) throw new Exception("Недостаточно прав");
        }
        ...
        async public Task<IResult> CreateUserAsync([FromBody] AuthenticateModel auth_model) =>
            await Task.Run(async () =>
            {
                try
                {
                    ...
                    if (IsUserContainsInQofRU(user)) throw new Exception("Пользователь с таким логином уже зарегистрировался");
                    if (IsUserContainsInDB(user)) throw new Exception("Пользователь с таким логином уже зарегистрировался");
